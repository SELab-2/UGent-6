...
public class EntityToJsonConverter {
  ...
  public GroupJson groupEntityToJson(GroupEntity groupEntity) {
    GroupJson group = new GroupJson(groupEntity.getId(), groupEntity.getName(),
        ApiRoutes.CLUSTER_BASE_PATH + "/" + groupEntity.getClusterId());
    GroupClusterEntity cluster = groupClusterRepository.findById(groupEntity.getClusterId())
        .orElse(null);
    ...
    List<UserReferenceJson> members = groupRepository.findGroupUsersReferencesByGroupId(
        groupEntity.getId()).stream().map(user ->
  ...
  public UserReferenceJson userEntityToUserReference(UserEntity user) {
    return new UserReferenceJson(user.getName() + " " + user.getSurname(), user.getEmail(),
        user.getId());
  }
  ...
  public CourseWithInfoJson courseEntityToCourseWithInfo(CourseEntity course, String joinLink) {
    ...
    List<UserReferenceJson> assistantsJson = assistants.stream()
        .map(this::userEntityToUserReference).toList();
  ...
  public CourseWithRelationJson courseEntityToCourseWithRelation(CourseEntity course,
  ...
  public ProjectResponseJson projectEntityToProjectResponseJson(ProjectEntity project,
    ...
    Integer completed = groupIds.stream().map(groupId -> {
      Long submissionId = submissionRepository.findLatestsSubmissionIdsByProjectAndGroupId(
          project.getId(), groupId);
      ...
      if (submission == null) {
        ...
        if (submission.getDockerAccepted() && submission.getStructureAccepted()) {
            return 1;
        }
      return 0;
    ...
    CourseUserEntity courseUserEntity = courseUserRepository.findById(
        new CourseUserId(course.getId(), user.getId())).orElse(null);
    ...
    return new ProjectResponseJson(
        new CourseReferenceJson(course.getName(), ApiRoutes.COURSE_BASE_PATH + "/" + course.getId(),
            course.getId()),
        project.getDeadline(),
