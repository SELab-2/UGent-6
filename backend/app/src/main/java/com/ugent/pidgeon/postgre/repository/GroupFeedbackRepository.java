...
public interface GroupFeedbackRepository extends JpaRepository<GroupFeedbackEntity, Long> {
    ...
    @Query(value = "SELECT grade FROM group_feedback WHERE group_id = ?1 AND project_id = ?2", nativeQuery = true)
    GroupFeedbackEntity getGroupFeedback(long groupId, long projectId);

    @Modifying
    @Transactional
    @Query(value = "UPDATE group_feedback SET grade = ?1 WHERE group_id = ?2 AND project_id = ?3", nativeQuery = true)
    int updateGroupScore(float grade, long groupId, long projectId);

    @Modifying
    @Transactional
    @Query(value = "INSERT INTO group_feedback (grade, group_id, project_id) VALUES (?1, ?2, ?3)", nativeQuery = true)
