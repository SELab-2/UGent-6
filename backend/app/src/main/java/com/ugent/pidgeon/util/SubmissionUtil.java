...
public class SubmissionUtil {
    ...
    public CheckResult<Void> hasAccesToGroupSubmissions(long groupId, long projectId, UserEntity user) {
        ProjectEntity project = projectUtil.getProjectIfExists(projectId);
        if (project == null) {
    ...
    public CheckResult<SubmissionEntity> canGetSubmission(long submissionId, UserEntity user) {
        ...
        if (hasAccesToGroupSubmissions(submission.getGroupId(), submission.getProjectId(), user).getStatus() == HttpStatus.OK) {
